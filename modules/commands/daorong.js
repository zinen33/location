...
module.exports.onLoad = async () => {
    const dir = __dirname + `/cache/daorong/datauser/`;
    const _dir = __dirname + `/cache/daorong/datauser/`;
    const __dir = __dirname + `/cache/daorong/cache/`;
...
module.exports.checkPath = function (type, senderID) {
    const pathGame = path.join(__dirname, 'cache', 'daorong', 'datauser', `${senderID}.json`);
    const pathGame_1 = require("./cache/daorong/datauser/" + senderID + '.json');
...
module.exports.image = async function(link) {
    ...
    let download = (await axios.get(link, { responseType: "arraybuffer" } )).data; 
        writeFileSync( __dirname + `/cache/daorong/cache/daorong.png`, Buffer.from(download, "utf-8"));
        images.push(createReadStream(__dirname + `/cache/daorong/cache/daorong.png`));
    return images
}
...
module.exports.run = async function ({ api, event, args, client,Threads,__GLOBAL, Users, Currencies,getText }) {
   const { senderID, messageID, threadID } = event;
    ...
    const pathData = path.join(__dirname, 'cache', 'daorong', 'datauser', `${senderID}.json`);
    switch (args[0]) {
        case 'register':
        case '-r': {
        ...
        case 'spin': {
            ...
            if(i == 3) {
                ...
                for (let i of data) { 
                    if(i != `${senderID}.json`) {
                        var o = require(`./cache/daorong/datauser/${i}`);
                        p.push(o)
        ...
        case 'xaydung': {
            ...
            var a = require(`./cache/daorong/datauser/${senderID}.json`);
