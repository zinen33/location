...
let {messageID, threadID, senderID} = event;
  ...
  var bans = JSON.parse(fs.readFileSync(__dirname + `/hethong/bans.json`)); //đọc nội dung file
  /*
  {warns: {}, banned: {tid: []}};
  */
	...
	var dtwbox = bans.warns[threadID];
  		...
  		for(let idtvw in dtwbox) {
  			...
  			for(let reasonwtv1 of dtwbox[idtvw]) {
  				if (reasonwtv1 == "[ 𝗕𝗔𝗡𝗦 ] Không có lý do nào được đưa ra") var reasonwtv = ".";
  		...
  		allwarn == "" ? api.sendMessage("[ 𝗕𝗔𝗡𝗦 ] Nhóm bạn chưa có ai bị ban.", threadID, messageID) : api.sendMessage("[ 𝗕𝗔𝗡𝗦 ] Danh sách những thành viên đã bị cấm vào nhóm:\n\n"+allwarn, threadID, messageID);
}